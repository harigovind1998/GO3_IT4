...
public class IntermediateHost {
	 ...
	 */
	private String getFileName(DatagramPacket initialPacket) {
			...
			switch (simulation) {
				...
				case 8:
					while(true) {
						...
						if((packet == 0) && clientNotSet) { //if the client TID has not been set, do so
							if(dup==1) {
								this.fileName = "./Server/" + getFileName(recievePacket);
							}else if(dup == 2) {
								this.fileName = "./Client/" + getFileName(recievePacket);
							}
				...
				case 9:
					while(true) {
						...
						if((packet == 0) && clientNotSet) { //if the client TID has not been set, do so
							...
							if(packetNumber == 0) {
								new File(fileName).setReadable(false);
							}
	...
	public IntermediateHost() {
		...
		System.out.println("Select Mode : Normal [0], Lost Packet [1], Delayed Packet [2], Duplicate Packet [3], Damage entire Packet [4], Damage WRQ/RRQ packet [5], Damage ACK/DATA packet[6], Lose last ACK packet[7], Delete File[8], Change File access[9]");
		simulation = sc1.nextInt();
		
		
		if(simulation != 0 && simulation !=7) {
			System.out.println("Which packet would you like to simulate the error (ignore for Damage WRQ/RRQ as it will be the first packet)");
			...
			if(simulation == 2) {
				...
				System.out.println("Delete: File created in the server in wrq[1], File created in the client by rrq[2]");
				dup = sc1.nextInt();
			}else if(simulation == 9) {
				System.out.println("Restrict File in: ./Server[1], ./Client[2]");
				dup = sc1.nextInt();
			}
		}
		sc1.close();
